export default {
    ...
    <div>
      ...
      <p>Completed Campaigns: {{ stats.completed_campaigns_ }}</p>
  ...
  async mounted() {
    try {
      const res = await fetch(`/sponsor-stats/${this.sponsor_id}`, {
        headers: {
  ...
  methods: {
    renderCharts() {
      const SponChart = document.getElementById('chart').getContext('2d');
      new Chart(SponChart, {
        type: 'bar',
        data: {
